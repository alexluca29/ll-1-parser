...
public class First {
    ...
    private void createFirst(Production production) {
        ...
        while (!ok) {
            ...
            if (index == production.getRight().split(" ").length)
                break;
            else
                current = production.getRight().split(" ")[index];
            ...
            if (grammar.isTerminal(current)) {
                ...
                // If we found epsilon in FIRST(non-terminal) and there is nothing left to parse
                // we add epsilon in FIRST(production)
                ...
                System.out.println("Error - Unknown symbol: " + current);
                break;
            }
        }
    }
